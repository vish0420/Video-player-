...
class _AudioPlayerScreenState extends State<AudioPlayerScreen> {
  ...
  Future<void> _openCurrent() async {
    ...
    try {
      final source = AudioSource.file(path, tag: MediaItem(id: path, title: current.title, artist: current.artist, album: current.album, duration: current.duration));
  ...
  Future<void> _previous() async {
    if (_player.position > const Duration(seconds: 3)) { await _player.seek(Duration.zero); return; }
    if (_index > 0) { setState(() => _index--); await _openCurrent(); }
  }

  void _sleep(Duration duration) {
    _sleepTimer?.cancel();
    if (duration == Duration.zero) { setState(() => _sleepRemaining = null); return; }
  ...
  Future<void> _showSleepTimer() async {
    final choice = await showModalBottomSheet<Duration>(context: context, builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  @override void dispose() { _sleepTimer?.cancel(); _player.dispose(); super.dispose(); }
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: SafeArea(child:Padding(padding:const EdgeInsets.all(24),child:Column(children:[
          ...
          const SizedBox(height:6), Text(current.artist,style:Theme.of(context).textTheme.bodyMedium), const SizedBox(height:20),
          StreamBuilder<Duration>(stream:_player.positionStream,builder:(_,p)=>StreamBuilder<Duration?>(stream:_player.durationStream,builder:(_,d){final pos=p.data??Duration.zero;final dur=d.data??current.duration;final max=dur.inMilliseconds>0?dur.inMilliseconds.toDouble():1.0;final value=pos.inMilliseconds.clamp(0,max.toInt()).toDouble();return Column(children:[Slider(value:value,min:0,max:max,onChanged:(v)=>_player.seek(Duration(milliseconds:v.toInt()))),Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[Text(_fmt(pos)),Text(_fmt(dur))])]);})),
          const SizedBox(height:12),
          StreamBuilder<PlayerState>(stream:_player.playerStateStream,builder:(_,s){final playing=s.data?.playing??false;return Row(mainAxisAlignment:MainAxisAlignment.center,children:[IconButton(iconSize:34,onPressed:_previous,icon:const Icon(Icons.skip_previous_rounded)),IconButton(iconSize:34,onPressed:()=>_player.seek(_player.position-const Duration(seconds:10)),icon:const Icon(Icons.replay_10_rounded)),IconButton(iconSize:72,onPressed:()=>playing?_player.pause():_player.play(),icon:Icon(playing?Icons.pause_circle_filled_rounded:Icons.play_circle_filled_rounded)),IconButton(iconSize:34,onPressed:()=>_player.seek(_player.position+const Duration(seconds:10)),icon:const Icon(Icons.forward_10_rounded)),IconButton(iconSize:34,onPressed:_next,icon:const Icon(Icons.skip_next_rounded))]);}),
          Row(mainAxisAlignment:MainAxisAlignment.center,children:[IconButton(tooltip:'Shuffle',icon:Icon(_shuffle?Icons.shuffle_on_rounded:Icons.shuffle_rounded),onPressed:()=>setState(()=>_shuffle=!_shuffle)),IconButton(tooltip:'Repeat',icon:Icon(_repeat?Icons.repeat_on_rounded:Icons.repeat_rounded),onPressed:()=>setState(()=>_repeat=!_repeat)),if(_sleepRemaining!=null)Padding(padding:const EdgeInsets.symmetric(horizontal:8),child:Text('Sleep ${_fmt(_sleepRemaining!)}'))]),
          const Spacer(), const Text('Swipe left/right to change tracks',style:TextStyle(fontSize:13)),
        ]))),
      ),
    );
  }
